...
export const uploadShippingProviderLogo = async (
    req,
    res
) => {
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
                message: "No provider logo was uploaded.",
            });
        }

        const logoUrl =
            `/uploads/shipping/providers/${req.file.filename}`;
        ...
        console.error(
            "Error uploading shipping provider logo:",
            error
        );
        ...
        return res.status(500).json({
            success: false,
            message:
                "Server error while uploading provider logo.",
        });
    }
};
